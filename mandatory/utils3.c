...
/*   Updated: 2025/02/21 18:07:04 by eamchart         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	images_failed(s_info *data)
{
	if (!data->wall_img || !data->wall_img1 || !data->empty_img
		|| !data->player_img || !data->collect_img || !data->door_img)
		free_all_mlx(data);
	if (!data->player_down[0] || !data->player_down[1] || !data->player_down[2]
		|| !data->player_down[3])
		free_all_mlx(data);
	if (!data->player_right[0] || !data->player_right[1]
		|| !data->player_right[2] || !data->player_right[3])
		free_all_mlx(data);
	if (!data->player_left[0] || !data->player_left[1] || !data->player_left[2]
		|| !data->player_left[3])
		free_all_mlx(data);
	if (!data->player_up[0] || !data->player_up[1] || !data->player_up[2]
		|| !data->player_up[3])
		free_all_mlx(data);
}
